...
const Header = () => {
  ...
  const navigationItems = [
    ...
    },
    {
      ...
      path: '/real-time-campaign-monitoring',
      icon: 'Activity',
      tooltip: 'Real-Time Campaign Monitoring'
    },
    {
      label: 'Subscribers',
      path: '/subscriber-analytics-dashboard',
      icon: 'Users',
      ...
      <div className="px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center space-x-8" role="navigation">
            {navigationItems.map((item) => (
              <button
                ...
                className={`group relative flex items-center space-x-2 px-4 py-2 rounded-lg nav-transition ${
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="hidden md:flex items-center space-x-3 px-3 py-2 bg-secondary-50 rounded-lg">
            ...
            <nav className="space-y-2" role="navigation">
              {navigationItems.map((item) => (
                <button
                  key={item.path}
                  onClick={() => handleNavigation(item.path)}
                  className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg nav-transition ${
                    location.pathname === item.path
                      ? 'bg-primary-50 text-primary border border-primary-100' :'text-text-secondary hover:text-text-primary hover:bg-secondary-50'
                  }`}
                  aria-current={location.pathname === item.path ? 'page' : undefined}
